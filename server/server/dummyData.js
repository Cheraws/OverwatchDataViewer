...
export default function () {
  Post.count().exec((err, count) => {
    ...
    //post 1
    let contents = [];
    let text1 =     "Recently in Overwatch, there has been a long ongoing debate about the effects on ladder from people maining characters," +
    ...
    let text2 =    "As we can see, while there is a significant number of mercy mains, the number is not more than half. " +  
                   "Genji and Dva seem to be the next most popular characters. Winston rounds out as the most popular main tank. " +
                   "In comparison, the number of one tricks, defined as over 80 percent playrate, is shown below.";


    let content = new Content({type:"text", text:text1});
    ...
    content = new Content({type:"text", text:text2});
    contents.push(content);
    ...
    content = new Content({type:"graph"});
    content.graphData.labels = label;
    content.graphData.numbers = number;
    content.graphData.title = "One Tricks";
    contents.push(content);
